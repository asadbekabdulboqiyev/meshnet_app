/*
 * Host tests (gcc): mesh_frame + relay logic — verified without hardware.
 * Run with:  make test  (or cc -Wall -I.. test_relay.c ../relay.c ../mesh_frame.c)
 */
#include <assert.h>
#include <stdio.h>
#include <string.h>
...
/* Build a plain TEXT frame (like a real Android emit). */
...
static int test_frame_roundtrip(void) {
    ...
    size_t n = build_text(buf, sender, target, 0x0102030405060708ULL, 6, "hello mesh");
    assert(n == MESH_HEADER_SIZE + 10);

    mesh_frame_t f;
    if (!mesh_frame_parse(buf, n, &f)) FAIL("parse failed");
    ...
    if (f.payload_len != 10 || memcmp(f.payload, "hello mesh", 10) != 0) FAIL("payload");
    if (f.pubkey != NULL) FAIL("TEXT must not carry a pubkey");

    /* Re-encode -> identical byte-for-byte */
    uint8_t out[MESH_MAX_FRAME];
    size_t m = mesh_frame_encode(&f, out, sizeof(out));
    if (m != n || memcmp(out, buf, n) != 0) FAIL("re-encode differs");
    tests++;
    return 0;
}
...
static int test_broadcast_target(void) {
    ...
    uint8_t bc[16] = {0};
    sender[0] = 0xAA;
    if (!mesh_frame_is_broadcast(bc)) FAIL("bc must be all zeros");
    if (mesh_frame_is_broadcast(sender)) FAIL("sender must not be broadcast");
    tests++;
    return 0;
}
...
static int test_frame_invalid(void) {
    ...
    mesh_frame_t f;
    if (mesh_frame_parse(buf, 0, &f)) FAIL("0 bytes should not parse");
    if (mesh_frame_parse(buf, MESH_HEADER_SIZE - 1, &f)) FAIL("short length should not parse");

    uint8_t bad[MESH_HEADER_SIZE + 4];
    memcpy(bad, buf, MESH_HEADER_SIZE);
    bad[0] = 0x00; /* wrong magic */
    if (mesh_frame_parse(bad, sizeof(bad), &f)) FAIL("bad magic should not parse");
    bad[0] = 0x4D; bad[1] = 0x4E;
    bad[3] = 0x63; /* unknown type */
    if (mesh_frame_parse(bad, sizeof(bad), &f)) FAIL("unknown type should not parse");
    tests++;
    return 0;
}
...
static int test_pair_frame_pubkey(void) {
    ...
    if (n != MESH_HEADER_SIZE + MESH_PUBKEY_LEN) FAIL("PAIR size");
...
static int test_relay_dedup_and_ttl(void) {
    ...
    uint8_t buf[MESH_MAX_FRAME];
    size_t n = build_text(buf, sender, target, 1000, 6, "message");
    mesh_frame_t f;
    ...
    int reason = -1;
    if (!relay_decide(&ctx, &f, 1000, &reason)) FAIL("first frame should be forwarded (reason=%d)", reason);

    /* After decrementing the TTL and forwarding again: now ttl=5 */
    relay_decrement_ttl(&f);
    if (f.ttl != 5) FAIL("ttl-- wrong");

    /* Duplicate: same (sender,seq) -> drop */
    mesh_frame_t f2 = f;
    if (relay_decide(&ctx, &f2, 1500, &reason)) FAIL("duplicate should be dropped");
    if (reason != RELAY_DROP_DUP) FAIL("dup reason");

    /* ttl==0 -> drop */
    f2.ttl = 0;
    if (relay_decide(&ctx, &f2, 2000, &reason)) FAIL("ttl=0 should be dropped");
    ...
    f2.msg_seq = 1001;
    f2.ttl = 5;
    if (!relay_decide(&ctx, &f2, 2500, &reason)) FAIL("new seq should be forwarded");
    tests++;
    return 0;
}
...
static int test_relay_seen_expiry(void) {
    ...
    uint8_t buf[MESH_MAX_FRAME];
    size_t n = build_text(buf, sender, target, 7, 6, "repeat");
    mesh_frame_t f;
    mesh_frame_parse(buf, n, &f);
    ...
    /* After 61s (TTL 60s) — the duplicate should now be accepted */
    if (relay_decide(&ctx, &f, RELAY_SEEN_TTL_MS + 1000, &reason)) {
        tests++;
        return 0;
    }
    FAIL("repeat should be accepted after 60s (reason=%d)", reason);
}
...
static int test_relay_cache_full(void) {
    ...
    /* Larger than the cache: 64+ messages with distinct seqs */
    for (uint64_t s = 0; s < RELAY_SEEN_SIZE + 20; s++) {
        uint8_t sender[16] = {0};
        sender[15] = (uint8_t)(s & 0xFF);
        sender[14] = (uint8_t)((s >> 8) & 0xFF);
        uint8_t buf[MESH_MAX_FRAME];
        size_t n = build_text(buf, sender, target, s, 6, "m");
        mesh_frame_t f;
        mesh_frame_parse(buf, n, &f);
        int reason = -1;
        if (!relay_decide(&ctx, &f, (uint32_t)s, &reason)) FAIL("new entry should be accepted");
    }
    tests++;
    return 0;
}
